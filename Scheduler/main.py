...
class Example(QWidget):
    ...
    def initDatabase(self):
        self.client = MongoClient('mongodb://jj:password@ds127341.mlab.com:27341/ondeck')
        ...
        self.fulltimers = self.db.fulltime
        self.fiveQers = self.db.downstairs
        self.vets = self.db.vets
        # print('hell')
        for admins in self.admins.find({}):
            print(admins['name'])
            admin_List.append(admins['name'])
        for operators in self.operators.find({}):
            print(operators['name'])
            name_List.append(operators['name'])

        for fulltimer in self.fulltimers.find({}):
            fulltime_List.append(fulltimer['name'])

        for fivQer in self.fiveQers.find({}):
            downstairs_List.append(fivQer['name'])

        for vet in self.vets.find({}):
            Trainer_list.append(vet['name'])
    ...
    def initUI(self):
        ...
        self.deleteEmployee = QPushButton('Delete', self)
        self.deleteEmployee.move(780, 24)
        self.deleteEmployee.clicked.connect(self.deleteE)
    ...
    def deleteE(self):
        ...
        if str(self.employeeRank.currentText()) == "5Q":
            downstairs = (str(self.employee.text()))
            if downstairs in downstairs_List:
                downstairs_List.remove(downstairs)
                self.fiveQers.remove({'name': downstairs})
                self.fiveQList.clear()
                downstairs_List.sort()
                for item in downstairs_List:
                    self.fiveQList.addItem(item)
        ...
        if str(self.employeeRank.currentText()) == "Fulltime":
            fulltime = (str(self.employee.text()))
            if fulltime in fulltime_List:
                fulltime_List.remove(fulltime)
                self.fulltimers.remove({'name': fulltime})
                self.fullTimeList.clear()
                fulltime_List.sort()
                for item in fulltime_List:
                    self.fullTimeList.addItem(item)
        ...
        if str(self.employeeRank.currentText()) == "Operator":
            operator = (str(self.employee.text()))
            if operator in name_List:
                name_List.remove(operator)
                self.operators.remove({'name':operator})
                self.operatorList.clear()
                name_List.sort()
                for item in name_List:
                    self.operatorList.addItem(item)
        ...
        if str(self.employeeRank.currentText()) == "Vets":
            operator = (str(self.employee.text()))
            if operator in Trainer_list:
                Trainer_list.remove(operator)
                self.vets.remove({'name': operator})
                self.vetList.clear()
                Trainer_list.sort()
                for item in Trainer_list:
                    self.vetList.addItem(item)
    ...
    #             umpire = sheet.cell_value(row, 6)
    #             gameStart = sheet.cell_value(row, 7)
    #             station = sheet.cell_value(row, 10)
    #             tx = sheet.cell_value(row, 11)
    #             visitTeam = sheet.cell_value(row, 4)
    #             homeTeam = sheet.cell_value(row, 5)
    ...
    def turnAround(self, x):
        ...
        for item in shortTurnAroundList:
                if item not in shortTurnAroundSet:
                    shortTurnAroundSet.append(item)
        for item in shortTurnAroundSet:
